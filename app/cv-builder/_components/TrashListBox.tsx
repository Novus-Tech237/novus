...
const TrashListBox = () => {
  ...
  // Apply the Document type to the filtered array
  const filteredDocuments = resumes?.filter((doc: Document) => {
    return doc.title?.toLowerCase()?.includes(search?.toLowerCase());
  });

  const onClick = (docId: string) => {
    router.push(`/cv-builder/document/${docId}/edit`);
  };
          ...
          <div className="text-sm">
            ...
            "
              ...
              {filteredDocuments?.map((doc: Document) => (
                <div
                  key={doc.id} // Ensure the key is of type number
                  role="button"
